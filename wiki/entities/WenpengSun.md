---
title: "Wenpeng Sun"
type: entity
tags: [研究者, 自动驾驶, 智能交通, 河北高速集团]
sources: [raw/09-archive/A Trajectory Data-Driven Personalized Autonomous Driving Decision System for Driving Simulators.pdf]
last_updated: 2026-05-12
---

## 定义

Wenpeng Sun（孙文鹏）是河北高速集团研究员，武汉理工大学兼职研究者。其研究方向为基于无人机航拍数据的个性化自动驾驶决策系统与驾驶模拟器验证平台。

## 关键信息

- **所属机构**: 河北高速集团（Hebei Expressway Group Co., Ltd.）
- **合作机构**: 武汉理工大学智能交通系统研究中心
- **代表工作**: 基于无人机航拍轨迹数据的个性化自动驾驶决策系统

## 关联连接

- [[PersonalizedAutonomousDriving]] — 其提出的个性化自动驾驶决策系统
- [[NengchaoLyu]] — 论文合作者
- [[YuZhang]] — 论文合作者
- [[摘要-trajectory-data-driven-personalized-autonomous-driving-decision-system]] — 论文来源摘要
