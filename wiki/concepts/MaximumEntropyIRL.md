...
| **TreeIRL** [Tomov et al., 2025] | MCTS 策略集近似 | MCTS 搜索图天然探索状态-动作空间 | 利用 MCTS 生成的候选轨迹集近似配分函数，结合深度神经网络评分 | 依赖环境模型准确性；状态空间维度受限 |
| **迭代 MaxEnt IRL（本文）** [Mehrdad et al., 2025] | 最优控制采样 + 迭代步长 | 通过求解最优控制问题生成信息性轨迹，无需大量随机样本 | 迭代寻找权重改进步长，单独调节每个观测对配分函数的有效性，最小观测即可学习 | 连续空间中的局部收敛性；最优控制求解的计算开销 |

**关键洞察**: 五种方法代表了连续域 MaxEnt IRL 的五种实现哲学：
- **解析近似**（CIOC）：利用局部数学性质，但假设过强
- **优化近似**（Opt-IRL）：用最优解代表分布，但计算爆炸
- **采样近似**（SMIRL）：利用领域先验生成样本，一次性完成
- **规划器内嵌近似**（路径积分）：利用规划器自身的搜索图，无需额外采样
- **最优控制驱动近似**（本文）：用最优控制问题生成信息性轨迹，迭代更新权重，最小观测即可学习

CIOC 和 Opt-IRL 的"更精确"近似附加了更强假设（最优性、局部凸性），在实际含噪声数据中反而可能引入偏差；SMIRL 的采样近似通过领域先验保证样本质量；路径积分方法则进一步将采样与规划器融合，无需独立的采样模块；本文方法则通过最优控制问题生成样本，相比随机采样获得更具信息量的轨迹，并通过迭代步长策略实现最小观测下的高效学习。
...
- [[摘要-cost-function-estimation-irl-minimal-observations]] — 迭代 MaxEnt IRL 代价函数估计（arXiv:2505.08619）
